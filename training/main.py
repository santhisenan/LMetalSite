...
for pdb_id in ID_list:
    if task == "Metal":
        all_protein_data[pdb_id] = (
            torch.load(root + f"{pdb_id}_node_feature.tensor"),
            torch.load(root + f"{pdb_id}_mask.tensor"),
            torch.load(root + f"{pdb_id}_label_mask.tensor"),
            torch.load(root + f"{pdb_id}_label.tensor"),
        )
    else:
        all_protein_data[pdb_id] = (
            torch.load(root + f"{pdb_id}_node_feature.tensor"),
            torch.load(root + f"{pdb_id}_mask.tensor"),
            torch.load(root + f"{pdb_id}_label_mask.tensor"),
            torch.load(root + task + "_label/" + f"{pdb_id}_label.tensor"),
        )
