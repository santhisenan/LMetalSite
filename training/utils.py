...
class TaskDataset:
    ...
    def __getitem__(self, idx):
        ...
        protein_node_features, protein_masks, protein_label_masks, labels = self.protein_data[pdb_id]
    ...
    def collate_fn(self, batch):
        ...
        protein_label_masks = torch.stack([item['PROTEIN_LABEL_MASK'] for item in batch], dim=0)
        labels = torch.stack([item['LABEL'] for item in batch], dim=0)

        return pdb_ids, protein_node_features, protein_masks, protein_label_masks, labels
    ...
    if train is not None:
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(train, train[label_name])):
            ...
            if model_class.__name__ in ['MetalSingle', 'MetalSite', "GraphSite"]:
                model = model_class(feature_dim, hidden_dim, num_encoder_layers, num_heads, augment_eps, dropout)
            elif model_class.__name__ in ['LSTM']:
                model = model_class(feature_dim, hidden_dim, num_encoder_layers, augment_eps, dropout)
            ...
            if args.train:
                for epoch in range(epochs):
                    ...
                    for i, data in enumerate(bar):
                        optimizer.zero_grad()
                        protein_node_features, protein_masks, protein_label_masks, y = [d.cuda() for d in data[1:]]
                        ...
                        if task == "Metal":
                            train_seq_preds = torch.masked_select(outputs, protein_label_masks.bool()) # 所有任务混在一起
                    ...
                    for data in tqdm(valid_dataloader):
                        ...
                        if task == "Metal":
                            for i in range(4):
                                valid_seq_y = torch.masked_select(y[:,i*maxlen:(i+1)*maxlen], protein_label_masks[:,i*maxlen:(i+1)*maxlen].bool())
                                valid_seq_preds = torch.masked_select(outputs[:,i*maxlen:(i+1)*maxlen], protein_label_masks[:,i*maxlen:(i+1)*maxlen].bool())
                    ...
                    if obj_max * (valid_metric[1]) > obj_max * best_valid_metric: # use AUPRC
                        ...
                        if task == "Metal":
                            ...
                            %(epoch,lr,train_loss,train_metric[0],train_metric[1],valid_metrics[0][0],valid_metrics[1][0],valid_metrics[2][0],valid_metrics[3][0],valid_metrics[0][1],valid_metrics[1][1],valid_metrics[2][1],valid_metrics[3][1]))
                        else:
                            Write_log(log,'[epoch %s] lr: %.6f, train_loss: %.6f, train_auc: %.6f, train_auprc: %.6f, valid_auc: %.6f, valid_auprc:%.6f'\
                            %(epoch,lr,train_loss,train_metric[0],train_metric[1],valid_metric[0],valid_metric[1]))
                    else:
                        ...
                        if task == "Metal":
                            ...
                            %(epoch,lr,train_loss,train_metric[0],train_metric[1],valid_metrics[0][0],valid_metrics[1][0],valid_metrics[2][0],valid_metrics[3][0],valid_metrics[0][1],valid_metrics[1][1],valid_metrics[2][1],valid_metrics[3][1],not_improve_epochs))
                        else:
                            Write_log(log,'[epoch %s] lr: %.6f, train_loss: %.6f, train_auc: %.6f, train_auprc: %.6f, valid_auc: %.6f, valid_auprc:%.6f, NIE +1 ---> %s'\
                            %(epoch,lr,train_loss,train_metric[0],train_metric[1],valid_metric[0],valid_metric[1],not_improve_epochs))
            ...
            if task == "Metal":
                for i in range(4):
                    oof.loc[val_index,metal_list[i]] = [valid_outputs[:,i*maxlen:(i+1)*maxlen][j, :len(oof.loc[val_idx,sequence_name])].tolist() for j,val_idx in enumerate(val_index)]
            else:
    ...
    if test is not None:
        ...
        with torch.no_grad():
            for data in tqdm(test_dataloader):
                ...
                if logit:
                    outputs = [model(protein_node_features, protein_masks).sigmoid() for model in models]
                else:
                    outputs = [model(protein_node_features, protein_masks) for model in models]
                ...
                if task == "Metal":
                    for i in range(4):
                        test_seq_y = torch.masked_select(y[:,i*maxlen:(i+1)*maxlen], protein_label_masks[:,i*maxlen:(i+1)*maxlen].bool())
                        test_seq_preds = torch.masked_select(outputs[:,i*maxlen:(i+1)*maxlen], protein_label_masks[:,i*maxlen:(i+1)*maxlen].bool())
        ...
        if task == "Metal":
            for i in range(4):
                sub[metal_list[i]] = [test_outputs[:,i*maxlen:(i+1)*maxlen][j, :len(sub.loc[j,sequence_name])].tolist() for j in range(len(sub))]
        else:
