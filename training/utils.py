...
class TaskDataset:
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "PROTEIN_LABEL_MASK": protein_label_masks,
            "LABEL": labels,
        }

    def collate_fn(self, batch):
        pdb_ids = [item["PDB_ID"] for item in batch]
        protein_node_features = torch.stack(
            [item["PROTEIN_NODE_FEAT"] for item in batch], dim=0
        )
        protein_masks = torch.stack([item["PROTEIN_MASK"] for item in batch], dim=0)
        protein_label_masks = torch.stack(
            [item["PROTEIN_LABEL_MASK"] for item in batch], dim=0
        )
...
def NN_train_and_predict(
    ...
    if train is not None:
        ...
        for fold, (train_index, val_index) in enumerate(
            kf.split(train, train[label_name])
        ):
            print("\n========== Fold " + str(fold + 1) + " ==========")

            train_dataset = TaskDataset(
                train.loc[train_index].reset_index(drop=True), protein_data, label_name
            )
            sampler = RandomSampler(
                train_dataset, replacement=True, num_samples=num_samples
            )  # when set to sampler, shuffle is False.
            ...
            valid_dataset = TaskDataset(
                train.loc[val_index].reset_index(drop=True), protein_data, label_name
            )
            ...
            if model_class.__name__ in ["MetalSingle", "MetalSite", "GraphSite"]:
                model = model_class(
                    feature_dim,
                    hidden_dim,
                    num_encoder_layers,
            ...
            elif model_class.__name__ in ["LSTM"]:
                model = model_class(
                    feature_dim, hidden_dim, num_encoder_layers, augment_eps, dropout
                )
            ...
            if args.train:
                for epoch in range(epochs):
                    ...
                    for i, data in enumerate(bar):
                        optimizer.zero_grad()
                        protein_node_features, protein_masks, protein_label_masks, y = [
                            d.cuda() for d in data[1:]
                        ]

                        outputs = model(protein_node_features, protein_masks)
                        ...
                        if logit:
                            outputs = (
                                outputs.sigmoid()
                            )  # outputs.shape = (batch_size, max_len)

                        train_pdb_ids.extend(data[0])

                        if task == "Metal":
                            train_seq_preds = torch.masked_select(
                                outputs, protein_label_masks.bool()
                            )  # 所有任务混在一起
                            train_seq_y = torch.masked_select(
                                y, protein_label_masks.bool()
                            )
                        else:
                            train_seq_preds = torch.masked_select(
                                outputs, protein_masks.bool()
                            )  # masked_select 会将bsize个样本的预测值展平，直接batch内所有样本合并成一个list了
                    ...
                    for data in tqdm(valid_dataloader):
                        ...
                        with torch.no_grad():
                            if logit:
                                outputs = model(
                                    protein_node_features, protein_masks
                                ).sigmoid()
                            else:
                                outputs = model(protein_node_features, protein_masks)
                        ...
                        if task == "Metal":
                            for i in range(4):
                                valid_seq_y = torch.masked_select(
                                    y[:, i * maxlen : (i + 1) * maxlen],
                                    protein_label_masks[
                                        :, i * maxlen : (i + 1) * maxlen
                                    ].bool(),
                                )
                                valid_seq_preds = torch.masked_select(
                                    outputs[:, i * maxlen : (i + 1) * maxlen],
                                    protein_label_masks[
                                        :, i * maxlen : (i + 1) * maxlen
                                    ].bool(),
                                )
                                valid_Y4[i].append(valid_seq_y.detach().cpu().numpy())
                                valid_preds4[i].append(
                                    valid_seq_preds.detach().cpu().numpy()
                                )
                        else:
                    ...
                    if task == "Metal":
                        ...
                        valid_metric = np.array(valid_metrics).mean(
                            axis=0
                        )  # [average_AUC, average_AUPR]
                    else:
                    ...
                    if (
                        obj_max * (valid_metric[1]) > obj_max * best_valid_metric
                    ):  # use AUPRC
                        if len(gpus) > 1:
                            torch.save(
                        ...
                        if task == "Metal":
                            Write_log(
                                log,
                                "[epoch %s] lr: %.6f, train_loss: %.6f, train_auc: %.6f, train_auprc: %.6f, valid_auc: %.6f,%.6f,%.6f,%.6f, valid_auprc:%.6f,%.6f,%.6f,%.6f"
                                % (
                                    epoch,
                                    lr,
                                    train_loss,
                            ...
                            Write_log(
                                log,
                                "[epoch %s] lr: %.6f, train_loss: %.6f, train_auc: %.6f, train_auprc: %.6f, valid_auc: %.6f, valid_auprc:%.6f"
                                % (
                                    epoch,
                                    lr,
                                    train_loss,
                        ...
                        if task == "Metal":
                            Write_log(
                                log,
                                "[epoch %s] lr: %.6f, train_loss: %.6f, train_auc: %.6f, train_auprc: %.6f, valid_auc: %.6f,%.6f,%.6f,%.6f, valid_auprc:%.6f,%.6f,%.6f,%.6f, NIE +1 ---> %s"
                                % (
                                    epoch,
                                    lr,
                                    train_loss,
                            ...
                            Write_log(
                                log,
                                "[epoch %s] lr: %.6f, train_loss: %.6f, train_auc: %.6f, train_auprc: %.6f, valid_auc: %.6f, valid_auprc:%.6f, NIE +1 ---> %s"
                                % (
                                    epoch,
                                    lr,
                                    train_loss,
            ...
            for data in tqdm(valid_dataloader):
                ...
                with torch.no_grad():
                    if logit:
                        outputs = model(protein_node_features, protein_masks).sigmoid()
                    else:
                        outputs = model(protein_node_features, protein_masks)
            ...
            if task == "Metal":
                ...
                Write_log(
                    log,
                    "[fold %s] best_valid_auc_avg: %.6f, best_valid_auprc_avg: %.6f, best_valid_auc: %.6f,%.6f,%.6f,%.6f, best_valid_auprc: %.6f,%.6f,%.6f,%.6f"
                    % (
                        fold,
                        valid_metric[0],
                        valid_metric[1],
                ...
                Write_log(
                    log,
                    "[fold %s] best_valid_auc: %.6f, best_valid_auprc: %.6f"
                    % (fold, valid_metric[0], valid_metric[1]),
                )
                all_valid_metric.append(valid_metric[1])  # AUPRC

            valid_outputs = np.concatenate(
                valid_outputs
            ...
            if task == "Metal":
                ...
                oof.loc[val_index, label_name] = [
                    valid_outputs[i, : len(oof.loc[val_idx, sequence_name])].tolist()
                    for i, val_idx in enumerate(val_index)
                ]
        ...
        if task == "Metal":
            ...
            Write_log(
                log,
                "all valid mean metric:%.6f,%.6f,%.6f,%.6f, avg metric over tasks:%.6f"
                % (
                    mean_valid_metric[0],
                    mean_valid_metric[1],
                    mean_valid_metric[2],
    ...
    if test is not None:
        ...
        with torch.no_grad():
            for data in tqdm(test_dataloader):
                ...
                if logit:
                    outputs = [
                        model(protein_node_features, protein_masks).sigmoid()
                        for model in models
                    ]
                else:
                    outputs = [
                        model(protein_node_features, protein_masks) for model in models
                    ]

                outputs = torch.stack(outputs, 0).mean(
                    0
                )  # 5个模型预测结果求平均,最终shape=(bsize, max_len)
                test_outputs.append(outputs.detach().cpu().numpy())

                if task == "Metal":
                    for i in range(4):
                        test_seq_y = torch.masked_select(
                            y[:, i * maxlen : (i + 1) * maxlen],
                            protein_label_masks[
                                :, i * maxlen : (i + 1) * maxlen
                            ].bool(),
                        )
                        test_seq_preds = torch.masked_select(
                            outputs[:, i * maxlen : (i + 1) * maxlen],
                            protein_label_masks[
                                :, i * maxlen : (i + 1) * maxlen
                            ].bool(),
        ...
        if task == "Metal":
            ...
            Write_log(
                log,
                "test_auc: %.6f,%.6f,%.6f,%.6f, test_auprc: %.6f,%.6f,%.6f,%.6f"
                % (
                    test_metrics[0][0],
                    test_metrics[1][0],
                    test_metrics[2][0],
            ...
            Write_log(
                log, "test_auc:%.6f, test_auprc:%.6f" % (test_metric[0], test_metric[1])
            )
        ...
        if task == "Metal":
            for i in range(4):
                sub[metal_list[i]] = [
                    test_outputs[:, i * maxlen : (i + 1) * maxlen][
                        j, : len(sub.loc[j, sequence_name])
                    ].tolist()
                    for j in range(len(sub))
                ]
        else:
            sub[label_name] = [
                test_outputs[i, : len(sub.loc[i, sequence_name])].tolist()
