...
def load_dataloader(result):
    ...
    dataloader = DataLoader(train_dataset, batch_size = BATCH_SIZE, collate_fn = train_dataset.collate_fn, shuffle = False, drop_last = False, num_workers = 4)
...
model = LMetalSite(feature_dim=NN_config['feature_dim'])  # replace YOUR_FEATURE_DIM with the correct dimension
model = model.to(DEVICE)
model.train()
...
for index,i in enumerate(['CA','MG',"MN",'ZN']):
    ...
    for epoch in range(EPOCHS):
        for batch_data in tqdm(dataloader):
            ...
            targets=torch.tensor([logit_output*mask_shape[1] for _ in range(mask_shape[0])])
